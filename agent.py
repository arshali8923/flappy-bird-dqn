...
class Agent:
    ...
    def run(self, is_training=True, render=False):
        ...
        # epsilon defined outside the if/else so it always exists,
        # even in testing mode (fixes UnboundLocalError)
        epsilon = 0
        ...
        for episode in itertools.count():
            ...
            while not terminated and episode_reward < self.reward_threshold:
                # epsilon-greedy (during training only; testing is fully greedy since epsilon=0)
                ...
                if is_training:
                    memory.append((state, action, next_state_t, reward_t, done))
                    steps += 1
...
if __name__ == "__main__":
    ...
    parser.add_argument("hyperparameters", help="Parameter set name from YAML")
